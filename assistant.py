...
PRESET_FILE = "presets.json" # <--- НОВА КОНСТАНТА

# -----------------------------------------------------------
# --- ФУНКЦИЯ АВТОУСТАНОВКИ ЗАВИСИМОСТЕЙ ---
# -----------------------------------------------------------

def install_dependencies():
    """Проверяет и устанавливает зависимости, перечисленные в requirements.txt."""
    try:
        # 1. СТРОГО ПРОВЕРЯЕМ, НЕ ВЫЗЫВАЯ ИСКЛЮЧЕНИЕ
        import PyQt5.QtWidgets 
        return True # Установлено
    except ImportError:
        print("Библиотека PyQt5 не найдена. Запуск установки...")
        
        # ... (логика поиска requirements.txt) ...
        requirements_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'requirements.txt')
        
        if not os.path.exists(requirements_path):
            # ... (обработка ошибки, если requirements.txt не найден) ...
            return False 
        ...
        try:
            ...
            print("\n✅ Установка PyQt5 завершена. ПОЖАЛУЙСТА, ПЕРЕЗАПУСТИТЕ приложение для запуска.")
            sys.exit(0) # Успешно установили, завершаем скрипт
            
        except subprocess.CalledProcessError:
            print("\n!!! КРИТИЧЕСКАЯ ОШИБКА УСТАНОВКИ !!!")
            print("Не удалось установить зависимости. Убедитесь, что 'pip' доступен.")
            sys.exit(1)
        except Exception as e:
            print(f"\nНеизвестная ошибка при установке: {e}")
            sys.exit(1)
...
if not install_dependencies():
    # Если install_dependencies вернула False (например, не нашла requirements.txt),
    # то мы не можем продолжать, и sys.exit(1) уже был вызван.
    pass 

# -----------------------------------------------------------
# --- ИМПОРТЫ PyQt5 И КОНСТАНТ ---
# -----------------------------------------------------------

# Теперь, когда мы уверены, что PyQt5 установлена, импортируем ее
...
# Импортируем все константы из файла config.py
from config import (
    SPRITE_DIR, IDLE_SPRITES, ANIMATION_SPEED_MS, 
    WALK_SPRITES_RIGHT, WALK_SPRITES_LEFT, WALK_DECISION_INTERVAL_MS, WALK_PIXELS_PER_STEP,
    WALK_MAX_DISTANCE,
    ### НОВИЙ КОД ДЛЯ МОРГАННЯ ###
...
class NoteDialog(QDialog):
    ...
    def get_data(self):
        ...
        return {
            'title': self.title_edit.text().strip(),             # <--- ТЕПЕР ПОВЕРТАЄМО 'title'
            'text': self.text_editor.toPlainText().strip(),
            'datetime': self.datetime_edit.dateTime(),
            'is_reminder_active': self.reminder_checkbox.isChecked() # <<< ДОДАНО
        }
...
class ReminderSetupDialog(QDialog):
    def __init__(self, parent=None):
        ...
        # self.radio_once = QCheckBox("Одноразово")
        # self.radio_once.setChecked(True) # За замовчуванням
        # self.radio_daily = QCheckBox("Щоденно")
        # self.radio_hourly = QCheckBox("Щогодинно")
        # self.radio_custom = QCheckBox("Кожні X хвилин")
...
class ManagePresetsDialog(QDialog):
    ...
    def select_action(self, action):
        ...
        # ВИПРАВЛЕННЯ: Використовуйте QDialog.Accepted замість self.Result.Accepted
        # Оскільки self успадковує QDialog, можна використовувати self.Accepted, але 
        # self.Result - це помилка. Краще використовувати QDialog.Accepted.
        
        # АБО:
        # self.done(QDialog.Accepted + (1 if action == 'open' else 2 if action == 'delete' else 3)) 
        
        # Оскільки QDialog.Accepted має значення 1, і ви додаєте 1, 2 або 3,
        # кінцеві коди будуть 2, 3, 4.

        # Краще використовувати самодостатні значення, щоб уникнути плутанини:
        # QDialog.Accepted (1) + 1 = 2 (Open)
        # QDialog.Accepted (1) + 2 = 3 (Delete)
        # QDialog.Accepted (1) + 3 = 4 (Edit)
        
        # АБО використовувати self.Accepted, оскільки воно успадковується
...
class ProgramSelectorDialog(QDialog):
    ...
    def load_programs(self):
        ...
        # 💡 УНІВЕРСАЛЬНИЙ ПОШУК: Використовуємо системні шляхи (PATH)
        # На Linux/macOS: /usr/bin, /bin, /usr/local/bin
        # На Windows: C:\Windows\System32, та шляхи, де часто є програми
        ...
        # ----------------------------------------------------
        # 3. ПОШУК У PROGRAM FILES (РЕКУРСИВНИЙ - ЗМІНА ТУТ)
        # ----------------------------------------------------
...
class VirtualAssistant(QWidget):
    def __init__(self):
        ...
        # --- НОВЕ СОСТОЯНИЕ ДЛЯ ПРИВІТАННЯ ---
        self.is_welcoming = True # <<< ФЛАГ ПРИВІТАННЯ

        # --- НОВЕ СОСТОЯНИЕ ДЛЯ МОРГАННЯ ---
        ...
        # --- НОВИЙ СТАН: Пресети ---
        self.presets = [] # <--- НОВИЙ СПИСОК ПРЕСЕТІВ
        ...
        self.direction = QPoint(0, 0) # Направление движения (dx, dy)
        
        # --- НОВЕ СОСТОЯНИЕ: Режим движения ---
        self.move_mode = 'free' 
        
        # --- НОВИЙ СТАН: Режим сну ---
        self.is_sleeping = False # <<< НОВА ЗМІННА

        self.initUI()

        # --- ВАЖЛИВО: ТАЙМЕРИ ЗАПУСКАЮТЬСЯ ПІСЛЯ ПРИВІТАННЯ ---
    ...
    def initUI(self):
        # 1. Настройка окна для прозрачности и отсутствия рамки
        self.setWindowFlags(
        ...
        # 2. Создание QLabel для отображения спрайтов
        self.label = QLabel(self)
        
        # Загружаем первый спрайт IDLE для инициализации размера
        if not self.load_sprite(IDLE_SPRITES[0]):
             return 

        # Размещаем помощника в начальной точке (справа внизу)
    ...
    def load_sprite(self, filename):
        """Загружает и отображает спрайт, подгоняя размер окна под него."""
        path = os.path.join(SPRITE_DIR, filename)
        
        if not os.path.exists(path):
            QMessageBox.critical(self, "Ошибка Спрайта", 
                                 f"Спрайт не найден: {path}. Проверьте пути в config.py.")
            return False
    ...
    def start_timers(self):
        """Запускает все необходимые таймеры: анимации и логики движения."""
        
        # 1. Таймер для покадровой анимации (будет изменять скорость)
        ...
        # 2. Таймер для логики принятия решений (движение)
    ...
    def show_note_dialog(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            # 2. 🔥 КРИТИЧНЕ ВИПРАВЛЕННЯ: Планування нагадування 🔥
    ...
    # def show_reminder(self, note_text, timer_to_remove):
    #     """Відображає нотатку як спливаюче нагадування."""
        
    #     title = "🔔 Нагадування від Помічника!"
    #     message = f"**Ваша замітка:**\n{note_text}"
        
    #     QMessageBox.information(self, title, message)
        
    #     # Видаляємо таймер зі списку після спрацювання
    #     if timer_to_remove in self.reminders:
    #         timer_to_remove.stop()
    #         self.reminders.remove(timer_to_remove)
    ...
    def schedule_flexible_reminder(self, text, start_dt, repetition, interval_ms=0, existing_id=None):
        ...
        if first_interval_ms <= 0 and repetition != 'once': # Одноразові вже відсіяли:
            ...
            # 💡 КРИТИЧНЕ ВИПРАВЛЕННЯ ДЛЯ ЦИКЛІЧНИХ
        ...
        def _trigger_action():
            ...
            # Для щоденного нагадування
                ...
                # 💡 ВИПРАВЛЕННЯ: Встановлюємо інтервал 24 години (86,400,000 мс)
            ...
            if interval > 0:
                # 💡 ВИПРАВЛЕННЯ: Це спрацює, якщо timer НЕ SingleShot.
                # Оскільки ви вже встановили timer.setSingleShot(repetition == 'once') 
                # перед запуском, для циклічних нагадувань він вже циклічний. 
                # Вам потрібно просто дозволити йому працювати на фіксованій швидкості.
                # Але, якщо ви використовуєте QTimer, встановлення інтервалу при кожному спрацюванні
                # є чистим рішенням.
    ...
    def save_reminders(self):
        ...
        for r in self.active_reminders:
            data_to_save.append({
                ...
                # НОВЕ: Зберігаємо точку відліку для циклічних (для коректного відновлення)
    ...
    def load_reminders(self, initial_load=True): # (initial_load - для уникнення діалогових вікон)
        ...
        try:
            ...
            # =========================================================
            # !!! НОВИЙ БЛОК: ОНОВЛЕННЯ ЛІЧИЛЬНИКА ID !!!
            # =========================================================
            # if data_to_load:
            #     # 1. Знаходимо максимальний ID серед завантажених нагадувань
            #     max_id = max(r_data.get('id', 0) for r_data in data_to_load)
                
            #     # 2. Встановлюємо лічильник на max_id + 1
            #     self.reminder_id_counter = max_id + 1

                
            # Змінено на (як у фінальному коді):
            # max_id_saved = data_to_load.get('max_id', 0)

            max_id_saved = data_from_file.get('max_id', 0) # ✅ Використовуємо data_from_file
            self.reminder_id_counter = max_id_saved + 1

            # ✅ ДОДАТИ: Оновлення списку нагадувань в інтерфейсі
            self.update_reminder_list_ui()

            # print(f"✅ Нагадування відновлено. Наступний ID встановлено на: {self.reminder_id_counter}")
        
        except json.JSONDecodeError as e: # ✅ ДОДАТИ: Обробка помилки формату JSON
        ...
        for item in items:
            ...
            try:
                if item.startswith(('http://', 'https://')):
                    ...
                    # ⚠️ ЗМІНА РОЗМІРУ: Ця логіка є заглушкою! 
    ...
    def _resize_application(self, app_path, width, height):
        ...
        # Додайте його, коли будете готові до додаткових залежностей.
    ...
    def toggle_sleep_mode(self):
        ...
        if self.is_sleeping:
            ...
            # 2. Встановлюємо спрайт сну (якщо є) або просто статичний IDLE
            # Тут ми просто залишаємо його на поточному спрайті, але в ідеалі
            # можна завантажити окремий спрайт "SLEEP_01".

            if not self.load_sprite("sleep/sleep_01.png"): # <<< ЗМІНА ТУТ
    ...
    def update_sprite(self):
        """Переключает текущий спрайт для создания анимации IDLE, WALK или BLINK."""
        ...
        # 3. Обновляем индекс и загружаем спрайт
    ...
    def make_move_decision(self):
        """Решает, начать ли движение, и устанавливает цель."""
        
        if self.is_sleeping: # <<< ПЕРЕВІРКА СТАНУ
            return

        # Ми тут припускаємо, що логіка переривання/продовження руху
        # має бути збережена з попередньої версії проекту:
        if random.random() < 0.7 and not self.is_moving: 
            # 70% шанс начать движение
            self.start_moving()
        elif self.is_moving:
            # С 30% шансом прервать движение (за умови, що start_moving не спрацював)
            self.stop_moving()


    def calculate_direction(self):
        """Вычисляет вектор направления к цели с учетом режима движения."""
        
        current_pos = self.pos()
        dx = self.target_pos.x() - current_pos.x()
        dy = self.target_pos.y() - current_pos.y()
        
        # По умолчанию: свободное движение
        dir_x = 0
        dir_y = 0
    ...
    def start_moving(self):
        """Начинает произвольное перемещение."""
        
        ### ФІКС: Якщо моргає, зупиняємо моргання перед рухом ###
        if self.is_blinking:
            self.stop_blink()
            
        self.is_moving = True
        
        # ... (Ваша логіка вибору цілі) ...
        screen_geo = QDesktopWidget().availableGeometry()
        current_pos = self.pos()
        ...
        # 2. НОВИЙ КОД: Встановлюємо, куди дивитися (СЮДИ!)
        ...
        # Вычисляем направление и запускаем таймер ходьбы
        self.calculate_direction()
        
        # --- ФИКС АНИМАЦИИ ХОДЬБЫ ---
        # 1. Ускоряем анимационный таймер для ходьбы 
        self.animation_timer.setInterval(80) 
        
        # 2. Форсируем немедленное переключение на WALK-анимацию
        ...
        self.walk_timer.start(20) # 20 мс = 50 кадров/сек для плавного движения


    def stop_moving(self):
        """Останавливает перемещение и переключает на IDLE-анимацию."""
        self.is_moving = False
        self.walk_timer.stop()
        self.direction = QPoint(0, 0)
        
        # --- ФИКС АНИМАЦИИ IDLE (ИЗМЕНЕННЫЕ СТРОКИ) ---
    ...
    def update_position(self):
        """Смещает помощника на один шаг в заданном направлении."""
        ...
        # Пересчитываем направление к оставшейся цели после шага
        self.calculate_direction()
    ...
    # --- НОВЫЙ МЕТОД: Переключает режим движения ---
    def toggle_move_mode(self):
        """Переключает режим движения: free -> horizontal -> vertical -> free."""
        ...
        # Останавливаем текущее движение, чтобы новый режим применился сразу
        if self.is_moving:
            self.stop_moving()
            # Перезапускаем таймер решения, чтобы она сразу начала новое движение
            self.decision_timer.start(WALK_DECISION_INTERVAL_MS)    
    ...
    def mouseMoveEvent(self, event):
        """Обрабатывает перетаскивание окна ЛКМ."""
    ...
    def closeEvent(self, event):
        ...
        # Перевіряємо та зупиняємо таймер руху/анімації (замініть імена на свої)
        ...
        # ДОДАТИ ЗУПИНКУ decision_timer та blink_decision_timer
        ...
        # !!! ДОДАТИ ЦЕЙ КОД СЮДИ: ЗУПИНКА ТАЙМЕРА ЗВОРОТНОГО ВІДЛІКУ !!!
        if hasattr(self, 'active_countdown_timer') and self.active_countdown_timer.isActive():
            self.active_countdown_timer.stop()
        # !!! КІНЕЦЬ БЛОКУ !!!

        # 3. !!! ДОДАТИ ЦЕЙ КОД СЮДИ: ЗУПИНКА ІНДИВІДУАЛЬНИХ ТАЙМЕРІВ НАГАДУВАНЬ !!!
        if hasattr(self, 'active_reminders') and self.active_reminders:
            ...
            for data in self.active_reminders:
                ...
                if 'timer' in data and isinstance(data['timer'], QTimer):
                   ...
                   # reminders_to_stop.append(data['timer'])

            # # Зупиняємо всі знайдені таймери
            # for timer in reminders_to_stop:
            #     if timer.isActive():
            #         timer.stop()
            
            # print(f"Інформація: {len(reminders_to_stop)} таймерів нагадувань зупинено.")
        # !!! КІНЕЦЬ БЛОКУ !!!

        # Якщо у вас є таймери, прив'язані до конкретних нагадувань, 
        # їх також потрібно перебрати та зупинити (наприклад, у self.reminders)
        
        # 3. Даємо згоду на закриття
        # print(">>> УСПІХ: closeEvent - Завершення процесу.")

        # 2. Даємо згоду на закриття
        event.accept()

        # 4. Гарантоване завершення додатку
        QCoreApplication.instance().quit() # <-- ДОДАЙТЕ ЦЕЙ РЯДОК

# --- ЗАПУСК ПРИЛОЖЕНИЯ ---
if __name__ == '__main__':
    if not os.path.exists(SPRITE_DIR):
        print(f"Ошибка: Папка '{SPRITE_DIR}' не найдена. Создайте ее и добавьте спрайты!")
        sys.exit(1)

    app = QApplication(sys.argv)
    ex = VirtualAssistant()
    
    # Завершаем, если не удалось загрузить начальный спрайт
    if not ex.isVisible():
        sys.exit(1)
